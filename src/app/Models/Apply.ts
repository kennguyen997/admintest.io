...
import isHangul from 'app/utils/isHangul';

yup.addMethod(yup.string, "isHangulOrEnglish", function isHangulOrEnglish(errKrMsg: string) {
  return this.test('isHangulOrEnglish', errKrMsg, (str: string) => {
    if (!isHangul(str)) {
      return false;
    }
...
export const ApplyYup = yup.object().shape({
  ...
  apply_budget: yup.string().isHangulOrEnglish(i18n.t(common.text_and_number_only)),
