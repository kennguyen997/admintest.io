...
import { ItemLikedListType } from 'app/Models/LikedList';
...
import { handleUnLiked } from 'app/Services/myPage';
type Prop = {
  dataLiked: Array<ItemLikedListType>[];
...
const DisplayMobileLiked: FC<Prop> = ({
  dataLiked,
  limitPageSize,
  setCurrentPage,
  hideHeader,
  ...
  const handleUnLike = async () => {
    setLoading(true);
    const response = await handleUnLiked({ arrayStreaming: selectedIdFilms });
  ...
  return (
    <div style={{ position: 'relative' }}>
      <ul className="list-film">
        {dataLiked.map((el: any) => {
          return (
            <li className="item-Liked-list" key={el._id} aria-hidden="true">
              <FilmList
...
export default DisplayMobileLiked;
